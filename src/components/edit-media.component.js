...
export default class EditMedia extends Component {
  ...
  componentDidMount() {
    axios.get('http://localhost:5000/medias/'+this.props.match.params.id)
      .then(response => {
        this.setState({
  ...
  onSubmit(e) {
    ...
    axios.post('http://localhost:5000/medias/update/' + this.props.match.params.id, media)
  ...
  render() {
    ...
    <div>
      ...
      <form onSubmit={this.onSubmit}>
        ...
        <div className="form-group">
          <input type="submit" value="Edit Media Log" className="btn btn-primary" />
        </div>
      </form>
    </div>
    )
